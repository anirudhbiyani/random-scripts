...
import boto3, json, dateutil.tz, sys
...
def main():
    ...
    res = boto3.client('iam', region_name='us-west-2')
    ...
    for user in users:
        ...
        user_groups = res.list_groups_for_user(UserName=username)
        skip = False
        ...
        access_keys = res.list_access_keys(UserName=username)['AccessKeyMetadata']
...
#        'Body': { 'Text': { 'Data': data}}})
    ...
    print 'AWS IAM Access Key Rotation Lambda Function (cron job) finished successfully at %s \n \n Deactivation Report:\n%s' % (finished, json.dumps(deactivated_report, indent=4, sort_keys=True))
